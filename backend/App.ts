...
export class App {
  ...
  addShape(
    ...
    do {
      ...
      isOverlapping =
        collisionMode === "on" ? this.checkSpawnCollision(newShape) : false;
      attempts++;
  ...
  checkSpawnCollision(newItem: any) {
    return this.shapes.some((existingItem: any) => {
      ...
      const x2 =
        existingItem.type === "circle" ? existingItem.x : existingItem.x + r2;
      const y2 =
        existingItem.type === "circle" ? existingItem.y : existingItem.y + r2;
